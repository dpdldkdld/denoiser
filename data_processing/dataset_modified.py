...
class Dataset:
    ...
    def parallel_audio_processing(self, clean_filename):
        ...
        noise = np.random.normal(0, 1.0, clean_audio.shape[0])
        hiss = butter_bandpass_filter(noise, 300, 7999, 16000)
        noiseInput = clean_audio + hiss * 1e-2
    ...
    def create_tf_record(self, *, prefix, subset_size, parallel=False):
        ...
        for i in range(0, len(self.clean_filenames), subset_size):
            ...
            tfrecord_filename = os.getenv('HOME')+'/data/records/' + prefix + '_' + str(counter) + '.tfrecords'
